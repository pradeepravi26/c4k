...
export default function VolunteerCheckOutSearch() {
  ...
  useEffect(() => {
    const fetchVolunteers = async () => {
      ...
      try {
        ...
        console.error("Error fetching checkout volunteers", error);
        setAllVolunteers([]);
      } finally {
        setLoading(false);
      }
    ...
    <div className="flex min-h-screen flex-col bg-background p-4">
      <div className="container max-w-4xl mx-auto py-8">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          {loading
            ? "Loading..."
            : `${filteredVolunteers.length} volunteer(s) available for check-out`}
        </p>
        ...
        {filteredVolunteers.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {filteredVolunteers.map((volunteer: Volunteer) => (
              <Link
                key={volunteer.id}
                href={`/volunteer/check-out/${volunteer.id}`}
                className="w-full"
              >
                <Button
                  variant="outline"
          ...
          !loading && (
            <div className="text-center py-12">
              <p className="text-xl text-muted-foreground">
                No volunteers available for check-out
              </p>
