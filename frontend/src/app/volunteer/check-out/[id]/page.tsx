...
export default function VolunteerCheckOut({
  params,
}: {
  ...
  if (loading || !resolvedParams) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-background p-4">
        <p className="text-lg">Loading volunteer information...</p>
      </div>
    );
  }
    ...
    <div className="flex min-h-screen flex-col bg-background p-4">
      <div className="container max-w-md mx-auto py-8">
        <div className="flex items-center mb-8">
          <Link href="/volunteer/check-in/search">
            <Button variant="ghost" size="icon" className="mr-2">
              <ArrowLeft className="h-5 w-5" />
            </Button>
          </Link>
          <h1 className="text-3xl font-bold">Volunteer Check-Out</h1>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {volunteer?.c4k_id || "N/A"} &middot; {volunteer?.full_name}
            </CardDescription>
          </CardHeader>
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                {status === "success"
                  ? "Volunteer has been successfully checked out. Redirecting to home page in 5 seconds."
                  : errorMessage ||
